...
void TPESC::FillGradientSingle(PolyCurveNetwork* curveNetwork, Eigen::MatrixXd& gradients, int i, int j,
...
void TPESC::FillGradientVectorDirect(PolyCurveNetwork* curveNetwork, Eigen::MatrixXd& gradients, double alpha,
                                     double beta) {
...
double TPESC::tpe_pair_pts(Vector3 p_x, Vector3 p_y, Vector3 tangent_x, double l_x, double l_y, double alpha,
                           double beta) {
...
Vector3 TPESC::grad_norm_proj_alpha(CurveVertex* i, CurveVertex* j, double alpha, double beta,
                                    CurveVertex* wrt) {
    Vector3 disp = i->Position() - j->Position();
    Vector3 T_i = i->Tangent();
...
Vector3 TPESC::grad_norm_proj_alpha(TangentMassPoint i, CurveVertex* j, double alpha, double beta,
                                    CurveVertex* wrt) {
    Vector3 disp = i.point - j->Position();
    Vector3 T_i = i.tangent;
    // Projection onto normal plane
...
Vector3 TPESC::grad_norm_proj_alpha(CurveVertex* i, TangentMassPoint j, double alpha, double beta,
                                    CurveVertex* wrt) {
    Vector3 disp = i->Position() - j.point;
    Vector3 T_i = i->Tangent();
    // Projection onto normal plane
...
Vector3 TPESC::grad_norm_proj_num(CurveVertex* i, CurveVertex* j, double alpha, double beta, CurveVertex* wrt,
                                  double h) {
